...
function arismed_dev_pay() {
  ...
  try {
    ...
    if ( $is_sbp ) {
      $response = create_sbp_qr($order);

      if ( isset($response['qrData']) ) {
        wp_send_json_success([
          'redirect' => $response['qrData'], 
          'order_id' => $order->get_id(),
        ]);
      } else {
        wp_send_json_error(['message' => 'Ошибка генерации QR-кода'], 500);
      }
    }

    // Если был гость — логиним и обновляем профиль
    if ( $was_guest && $customer_id > 0 ) {
      ...
      if ( $u ) {
        ...
        if ( $display === '' ) {
          $display = $email; // fallback если имя не ввели
        }
    ...
    if ($is_sbp) {
      ...
      exit; // Останавливаем выполнение после редиректа
    }
